...
use crate::{ActiveEnemies, Enemy, FromEnemy, Laser, Materials, MAX_ENEMIES, MAX_FORMATION_MEMBERS, SCALE, Speed, TIME_STEP, WinSize};
    ...
    fn make(&mut self, win_size: &WinSize) -> Formation {
        match (
            ...
            self.current_formation_members >= MAX_FORMATION_MEMBERS) {
                ...
                // if still within formation count
                (Some(tmpl), false) => {
                    self.current_formation_members += 1;
                    tmpl.clone()
                }
...
fn enemy_fire( mut commands: Commands, materials: Res<Materials>, enemy_query: Query<&Transform, With<Enemy>>)
{
    ...
    for &tf in enemy_query.iter() {
        ...
        commands.spawn_bundle(SpriteBundle{
            ...
            transform: Transform {
                ...
                // -SCALE will flit the enemy laser by 180 degree
...
fn enemy_laser_movement(mut commands: Commands, win_size: Res<WinSize>,
                        mut laser_query: Query<(Entity, &Speed,
                        &mut Transform), (With<Laser>, With<FromEnemy>)>)
{
...
fn enemy_movement(mut query: Query<(&mut Transform, &Speed, &mut Formation), With<Enemy>>)
{
    ...
    for (mut tf, speed, mut formation) in query.iter_mut() {
        ...
        // compute next angle
        let dir = if formation.start.0 > 0. {
            1.
        } else {
            -1.
        };
